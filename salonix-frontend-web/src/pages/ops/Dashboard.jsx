...
import { AlertTriangle, CheckCircle, RefreshCw, Activity, Users, DollarSign, Bell } from 'lucide-react';
...
const OpsDashboard = () => {
  ...
  const { totals, mrr_estimated, notification_daily } = metrics;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
          ...
          <p className="text-3xl font-bold text-white">{totals.active_tenants}</p>
          <div className="mt-2 text-yellow-400 text-xs flex items-center">
            <span>{totals.trials_expiring_7d} trials expirando em 7d</span>
          </div>
        </div>
        ...
        <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
          ...
          <div className="mt-2 text-gray-500 text-xs">
            {mrr_estimated.breakdown.pro?.count || 0} Pro · {mrr_estimated.breakdown.standard?.count || 0} Std · {mrr_estimated.breakdown.basic?.count || 0} Basic
          </div>
        </div>

        <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-gray-400 text-sm font-medium">Alertas Abertos</h3>
            <AlertTriangle size={16} className="text-red-500" />
          </div>
          <p className="text-3xl font-bold text-white">{totals.alerts_open}</p>
          <div className="mt-2 text-gray-500 text-xs flex items-center">
            <Link to="/ops/support" className="hover:text-purple-400">Ver todos &rarr;</Link>
          </div>
        </div>

        <div className="bg-gray-800 p-6 rounded-lg border border-gray-700">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-gray-400 text-sm font-medium">Notificações (24h)</h3>
            <Bell size={16} className="text-blue-500" />
          </div>
          <p className="text-3xl font-bold text-white">
             {notification_daily.length > 0 ? notification_daily[notification_daily.length - 1].total : 0}
          </p>
            ...
            {notification_daily.map((day) => {
              ...
              return (
                <div key={day.date} className="flex flex-col items-center flex-1 group relative">
                   <div className="w-full bg-gray-700 rounded-t hover:bg-purple-900 transition-colors relative" style={{ height: `${heightPercent}%`, minHeight: '4px' }}>
                      <div className="opacity-0 group-hover:opacity-100 absolute -top-10 left-1/2 transform -translate-x-1/2 bg-black text-white text-xs py-1 px-2 rounded whitespace-nowrap z-10 pointer-events-none">
                        {day.total} msgs
                        <br />
                        SMS: {day.channels?.sms || 0} | WA: {day.channels?.whatsapp || 0}
                      </div>
                   </div>
                   <span className="text-xs text-gray-500 mt-2 rotate-0 text-center">{dateLabel}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="bg-gray-800 rounded-lg border border-gray-700 p-6">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-semibold text-white flex items-center gap-2">
             ...
             <Link to="/ops/support" className="text-xs text-purple-400 hover:text-purple-300">Ver Central</Link>
          </div>

          <div className="space-y-4">
            {alertsLoading && <div className="text-gray-500 text-sm">Carregando alertas...</div>}
            ...
            {alerts.slice(0, 5).map((alert) => (
              <div key={alert.id} className="bg-gray-900/50 p-3 rounded border border-gray-700/50 hover:border-gray-600 transition-colors">
                ...
                <p className="text-sm text-gray-300 mt-1 line-clamp-2">{alert.message}</p>
