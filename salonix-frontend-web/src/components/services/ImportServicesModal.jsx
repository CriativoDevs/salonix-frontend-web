...
function ImportServicesModal({
    ...
    try {
      ...
      if (!result?.success) {
        setError(
          result?.error || {
            message: t(
              'services.import.errors.generic',
      ...
      if (!result?.success) {
        setError(
          result?.error || {
            message: t(
              'services.import.errors.generic',
              'Não foi possível importar os serviços.'
            ),
          }
        );
        return;
  ...
  const footer =
    step === STEP_DONE ? (
      <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
        ...
        >
          {t('services.import.actions.import_another', 'Importar outro ficheiro')}
        </button>
        <button
          type="button"
          onClick={onClose}
      ...
      <div className="flex justify-end gap-3">
        ...
        {step === STEP_PREVIEW ? (
          ...
          >
            {submitting
              ? t('services.import.actions.confirming', 'A importar...')
              : t('services.import.actions.confirm', 'Confirmar importação')}
          </button>
        ) : (
          <button
            type="button"
          ...
          >
            {submitting
              ? t('services.import.actions.previewing', 'A validar...')
              : t('services.import.actions.preview', 'Pré-visualizar')}
          </button>
        )}
      </div>
    );
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={t('services.import.title', 'Importar serviços')}
      description={t(
        'services.import.description',
        'Importe serviços a partir de um ficheiro CSV. O ficheiro é validado antes de gravar.'
      )}
      footer={footer}
    >
      <div className="space-y-4">
        <button
          ...
          className="text-sm font-medium text-brand-primary hover:underline"
        >
          {t('services.import.actions.download_template', 'Baixar modelo CSV')}
        </button>
          ...
          <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800">
            <p>
              {t(
                ...
                'Serviços importados com sucesso!'
              )}
            </p>
            {summary ? (
              <p className="mt-2 text-xs text-emerald-700">
                {t(
                  'services.import.summary.created',
                  'Criados: {{count}}',
          ...
          <>
            <div>
              <label className="block text-sm font-medium text-brand-surfaceForeground">
                {t('services.import.fields.file', 'Ficheiro CSV')}
              </label>
              <input
                ref={fileInputRef}
                type="file"
                accept=".csv,text/csv"
                aria-label={t('services.import.fields.file', 'Ficheiro CSV')}
            ...
            {step === STEP_PREVIEW && summary ? (
              <div className="space-y-3 rounded-lg border border-brand-border bg-brand-light/40 p-4">
                <p className="text-sm font-medium text-brand-surfaceForeground">
                  {t('services.import.summary.title', 'Resumo da pré-visualização')}
                </p>
                <ul className="space-y-1 text-sm text-brand-surfaceForeground/80">
                  <li>
                    {t('services.import.summary.created', 'Criados: {{count}}', {
                      count: summary.created ?? 0,
                    })}
                  </li>
                  <li>
                    {t('services.import.summary.updated', 'Atualizados: {{count}}', {
                      count: summary.updated ?? 0,
                    })}
                  </li>
                  <li>
                    {t('services.import.summary.skipped', 'Ignorados: {{count}}', {
                  ...
                  <div className="space-y-1 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
                    {errorsList.map((rowError, index) => (
                      <p key={`${rowError.line}-${index}`}>
                        {t(
                          'services.import.summary.row_error',
                          'Linha {{line}}: {{error}}',
