...
function ImportAppointmentsModal({
  open,
  onClose,
  onSubmit,
  onDownloadTemplate,
  ...
  const footer =
    step === STEP_DONE ? (
      <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
        ...
        >
          {t('bookings.import.actions.import_another', 'Importar outro ficheiro')}
        </button>
        <button
          type="button"
          onClick={onClose}
      ...
      <div className="flex justify-end gap-3">
        ...
        {step === STEP_PREVIEW ? (
          ...
          >
            {submitting
              ? t('bookings.import.actions.confirming', 'A importar...')
              : t('bookings.import.actions.confirm', 'Confirmar importação')}
          </button>
        ) : (
          <button
            type="button"
          ...
          >
            {submitting
              ? t('bookings.import.actions.previewing', 'A validar...')
              : t('bookings.import.actions.preview', 'Pré-visualizar')}
          </button>
        )}
      </div>
    );
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={t('bookings.import.title', 'Importar agendamentos')}
      description={t(
        'bookings.import.description',
        'Importe agendamentos futuros a partir de um ficheiro CSV. O ficheiro é validado antes de gravar.'
      )}
      footer={footer}
    >
      <div className="space-y-4">
        <button
          ...
          className="text-sm font-medium text-brand-primary hover:underline"
        >
          {t('bookings.import.actions.download_template', 'Baixar modelo CSV')}
        </button>
        ...
        {step === STEP_DONE ? (
          <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800">
            <p>
              {t(
                'bookings.import.success',
                'Agendamentos importados com sucesso!'
              )}
            </p>
            {summary ? (
              <p className="mt-2 text-xs text-emerald-700">
                {t(
                  'bookings.import.summary.created',
                  'Criados: {{count}}',
          ...
          <>
            <div>
              <label className="block text-sm font-medium text-brand-surfaceForeground">
                {t('bookings.import.fields.file', 'Ficheiro CSV')}
              </label>
              <input
                ref={fileInputRef}
                type="file"
                accept=".csv,text/csv"
                aria-label={t('bookings.import.fields.file', 'Ficheiro CSV')}
            ...
            {step === STEP_PREVIEW && summary ? (
              <div className="space-y-3 rounded-lg border border-brand-border bg-brand-light/40 p-4">
                <p className="text-sm font-medium text-brand-surfaceForeground">
                  {t('bookings.import.summary.title', 'Resumo da pré-visualização')}
                </p>
                <ul className="space-y-1 text-sm text-brand-surfaceForeground/80">
                  <li>
                    {t('bookings.import.summary.created', 'Criados: {{count}}', {
                      count: summary.created ?? 0,
                    })}
                  </li>
                  <li>
                    {t('bookings.import.summary.updated', 'Atualizados: {{count}}', {
                      count: summary.updated ?? 0,
                    })}
                  </li>
                  <li>
                    {t('bookings.import.summary.skipped', 'Ignorados: {{count}}', {
                  ...
                  <div className="space-y-1 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
                    {errorsList.map((rowError, index) => (
                      <p key={`${rowError.line}-${index}`}>
                        {t(
                          'bookings.import.summary.row_error',
                          'Linha {{line}}: {{error}}',
...
export default ImportAppointmentsModal;
