...
function ImportStaffModal({
      ...
      if (!result?.success) {
        setError(
          result?.error || {
            message: t(
              'team.import.errors.generic',
              'Não foi possível importar o staff.'
            ),
          }
        );
        return;
  ...
  const footer =
    step === STEP_DONE ? (
      <div className="flex flex-col gap-3 sm:flex-row sm:justify-end">
        ...
        >
          {t('team.import.actions.import_another', 'Importar outro ficheiro')}
        </button>
        <button
          type="button"
          onClick={onClose}
      ...
      <div className="flex justify-end gap-3">
        ...
        {step === STEP_PREVIEW ? (
          ...
          >
            {submitting
              ? t('team.import.actions.confirming', 'A importar...')
              : t('team.import.actions.confirm', 'Confirmar importação')}
          </button>
        ) : (
          <button
            type="button"
          ...
          >
            {submitting
              ? t('team.import.actions.previewing', 'A validar...')
              : t('team.import.actions.preview', 'Pré-visualizar')}
          </button>
        )}
      </div>
    );
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={t('team.import.title', 'Importar staff')}
      description={t(
        'team.import.description',
        'Importe membros da equipa a partir de um ficheiro CSV. O ficheiro é validado antes de gravar.'
      )}
      footer={footer}
    >
      <div className="space-y-4">
        <button
          ...
          className="text-sm font-medium text-brand-primary hover:underline"
        >
          {t('team.import.actions.download_template', 'Baixar modelo CSV')}
        </button>
        ...
        {step === STEP_DONE ? (
          <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800">
            <p>
              {t(
                'team.import.success',
                'Staff importado com sucesso!'
              )}
            </p>
            {summary ? (
              <p className="mt-2 text-xs text-emerald-700">
                {t(
                  'team.import.summary.created',
                  'Criados: {{count}}',
          ...
          <>
            <div>
              <label className="block text-sm font-medium text-brand-surfaceForeground">
                {t('team.import.fields.file', 'Ficheiro CSV')}
              </label>
              <input
                ref={fileInputRef}
                type="file"
                accept=".csv,text/csv"
                aria-label={t('team.import.fields.file', 'Ficheiro CSV')}
            ...
            {step === STEP_PREVIEW && summary ? (
              <div className="space-y-3 rounded-lg border border-brand-border bg-brand-light/40 p-4">
                <p className="text-sm font-medium text-brand-surfaceForeground">
                  {t('team.import.summary.title', 'Resumo da pré-visualização')}
                </p>
                <ul className="space-y-1 text-sm text-brand-surfaceForeground/80">
                  <li>
                    {t('team.import.summary.created', 'Criados: {{count}}', {
                      count: summary.created ?? 0,
                    })}
                  </li>
                  <li>
                    {t('team.import.summary.updated', 'Atualizados: {{count}}', {
                      count: summary.updated ?? 0,
                    })}
                  </li>
                  <li>
                    {t('team.import.summary.skipped', 'Ignorados: {{count}}', {
