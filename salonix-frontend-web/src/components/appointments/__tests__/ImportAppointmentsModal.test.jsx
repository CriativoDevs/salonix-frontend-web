...
describe('ImportAppointmentsModal', () => {
  ...
  it('previews with dry_run before allowing confirmation', async () => {
    ...
    expect(screen.getByText(/Atualizados: 1/)).toBeInTheDocument();
    expect(
  ...
  it('confirms the import after a successful preview', async () => {
    ...
    expect(
      await screen.findByText('Agendamentos importados com sucesso!')
    ).toBeInTheDocument();
  });
  ...
  it('shows row errors returned by the dry run and blocks confirmation', async () => {
    const onSubmit = jest.fn().mockResolvedValue({
      ...
      summary: {
        ...
        updated: 0,
        skipped: 1,
        errors: [{ line: 3, error: 'Horário indisponível' }],
      },
    });
    ...
    expect(
      await screen.findByText(/Linha 3: Horário indisponível/)
    ).toBeInTheDocument();
  });
